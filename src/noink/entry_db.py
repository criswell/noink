...
class EntryDB:
    ...
    def get_parent_info(self, e):
        """
        Given an entry, return a tuple containing its parent info.

        :param e: An entry to find children for. Can be an integer for the
                  entry id or an entry object.

        :returns: A tuple containing (top, prev, next). If any one is None,
                  that one does not exist.
        """
        entry = e
        if type(e) is IntType:
            entry = self.find_by_id(e)

        top = None
        children = []
        if entry.parent is IntType:
            top = self.find_by_id(entry.parent)
            children = self.get_parent_info(top)

        prev_e = None
        next_e = None
        if children:
            i = children.index(entry)
            if i > 0:
                prev_e = children[i-1]
            if i < len(children) - 1:
                next_e = children(i+1)

        return (top, prev_e, next_e)
